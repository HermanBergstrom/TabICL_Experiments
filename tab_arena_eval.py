...
if __name__ == "__main__":
    ...
    for task_id in task_ids:
        ...
        if not os.path.exists(f'tabarena_results/{dataset.name}'):
            os.makedirs(f'tabarena_results/{dataset.name}')
        ...
        for repeat in range(tabarena_repeats):
            for fold in range(folds):
                ...
                if visualize:
                    create_representation_plots(sorted_indices, 20, rollout['row_embeddings'][0][0][:X_train.shape[0]], y_train_binary, seed, save_directory=f'tabarena_results/{dataset.name}/representation_visualizations')
        ...
        #save results_dict
        with open(f'tabarena_results/{dataset.name}/results.pkl', 'wb') as f:
